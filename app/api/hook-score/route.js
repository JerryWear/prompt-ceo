...
import { buildHookScorerPrompt } from '../../prompt-engine-v3/ad-system/hookScorer.js'
...
export async function POST(req) {
  try {
    ...
    const { hookText, platform, productContext, targetCustomer } = await req.json()
    if (!hookText?.trim()) {
      return NextResponse.json({ status: 'error', message: 'hookText required' }, { status: 400 })
    }

    const apiKey = String(process.env.XAI_API_KEY || '').replace(/^Bearer\s+/i, '').replace(/^"+|"+$/g, '').trim()
    const prompt = buildHookScorerPrompt({ hookText, platform, productContext, targetCustomer })
    ...
    const grokRes = await fetch('https://api.x.ai/v1/chat/completions', {
      ...
      body: JSON.stringify({
        model: 'grok-3-fast',
        messages: [
          { role: 'system', content: 'You are a senior ad performance analyst. Output ONLY valid JSON. No markdown. Start with {.' },
    ...
    try {
      ...
      score = JSON.parse(m ? m[0] : raw)
    } catch {
      return NextResponse.json({ status: 'error', message: 'Failed to parse score' }, { status: 500 })
    }

    await db.from('app_users').update({ credits: userRow.credits - CREDIT_COST }).eq('id', user.id)

    return NextResponse.json({ status: 'success', score, creditsUsed: CREDIT_COST, creditsRemaining: userRow.credits - CREDIT_COST })
  } catch (err) {
