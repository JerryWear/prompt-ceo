...
import { buildTalkingHeadScriptPrompt } from '../../prompt-engine-v3/ad-system/talkingHeadScript.js'
...
export async function POST(req) {
  try {
    ...
    const prompt = buildTalkingHeadScriptPrompt(adConfig)

    const grokRes = await fetch('https://api.x.ai/v1/chat/completions', {
      method: 'POST', headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${apiKey}` },
      body: JSON.stringify({ model: 'grok-3-fast', messages: [
        { role: 'system', content: 'You are a professional video scriptwriter. Output ONLY valid JSON. No markdown. Start with {.' },
    ...
    let script
    try { const m = raw.match(/\{[\s\S]*\}/); script = JSON.parse(m ? m[0] : raw) }
    catch { return NextResponse.json({ status: 'error', message: 'Failed to parse script' }, { status: 500 }) }

    await db.from('app_users').update({ credits: userRow.credits - CREDIT_COST }).eq('id', user.id)
    return NextResponse.json({ status: 'success', script, creditsUsed: CREDIT_COST, creditsRemaining: userRow.credits - CREDIT_COST })
  } catch (err) {
