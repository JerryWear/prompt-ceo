...
import { buildUGCBriefPrompt } from '../../prompt-engine-v3/ad-system/ugcBrief.js'
...
export async function POST(req) {
  try {
    ...
    const prompt = buildUGCBriefPrompt(adConfig)
    ...
    const grokRes = await fetch('https://api.x.ai/v1/chat/completions', {
      ...
      body: JSON.stringify({
        model: 'grok-3-fast',
        messages: [
          { role: 'system', content: 'You are a senior UGC creative director. Output ONLY valid JSON. No markdown. Start with {.' },
    ...
    try {
      ...
      brief = JSON.parse(m ? m[0] : raw)
    } catch {
      return NextResponse.json({ status: 'error', message: 'Failed to parse brief' }, { status: 500 })
    }
