...
import { generateVideo } from '../../../app/video-system/orchestrator.js'
...
function resolveVideoDuration(progressionLevel) {
  if (progressionLevel === 'payoff' || progressionLevel === 'tension') return 10
  return 5
}

// ─── Ad video prompt builders ─────────────────────────────────────────────────

function buildProductAdVideoPrompt({ productName, adStyle, platform }) {
  const motionMap = {
  ...
  return `Commercial advertising video for: ${productName}
...
function buildPersonalBrandAdVideoPrompt({ creatorNiche, adGoal, platform }) {
  ...
  return `Personal brand advertising video for a ${creatorNiche} creator.
...
export async function POST(req) {
  try {
    ...
    const preferredProvider  = String(body?.preferredProvider || '').trim() || null
    ...
    if (mode === 'product_ad') {
      const adPrompt = buildProductAdVideoPrompt({
      ...
      const adPrompt = buildPersonalBrandAdVideoPrompt({
    ...
    const ratio      = mode === 'product_ad' ? '1280:720' : '720:1280'
