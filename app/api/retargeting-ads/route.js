...
import { buildRetargetingAdsPrompt } from '../../prompt-engine-v3/ad-system/retargetingAds.js'
...
export async function POST(req) {
  try {
    ...
    const prompt = buildRetargetingAdsPrompt(adConfig)
    ...
    const grokRes = await fetch('https://api.x.ai/v1/chat/completions', {
      ...
      body: JSON.stringify({
        model: 'grok-3-fast',
        messages: [
          { role: 'system', content: 'You are a senior performance media buyer specialising in retargeting. Output ONLY valid JSON. No markdown. Start with {.' },
    ...
    try {
      ...
      pack = JSON.parse(m ? m[0] : raw)
    } catch {
      return NextResponse.json({ status: 'error', message: 'Failed to parse retargeting pack' }, { status: 500 })
    }

    await db.from('app_users').update({ credits: userRow.credits - CREDIT_COST }).eq('id', user.id)
    return NextResponse.json({ status: 'success', pack, creditsUsed: CREDIT_COST, creditsRemaining: userRow.credits - CREDIT_COST })
  } catch (err) {
    console.error('Retargeting ads error:', err)
