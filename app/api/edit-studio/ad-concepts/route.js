...
const SYSTEM_PROMPT = `You are a senior performance creative director at a top SaaS marketing agency. You write ads that convert — not ads that describe software. Every script line must be short enough to say out loud in one breath. Every hook must name the specific product. No filler words. No hedging. No generic SaaS copy. Write as if you are handing this to a video production team tomorrow.`
...
function buildConceptPrompt(understandingData, creativeStrategy) {
  ...
  const productContext = [
    ...
    understandingData?.business_description
      ? `Business description: ${understandingData.business_description}`
      : '',
    understandingData?.key_messages?.length
      ? `Key messages:\n${understandingData.key_messages.map(m => `- ${m}`).join('\n')}`
      : '',
  ].filter(Boolean).join('\n\n')
  ...
  const strategyContext = [
    ...
    strategy.hook_strategy
      ...
      : '',
    strategy.opening_hook_example
      ? `Opening hook example: ${strategy.opening_hook_example}`
      : '',
    strategy.cta
      ? `CTA: ${strategy.cta}`
      : '',
...
For each concept generate: ad_type, hook_archetype, hook, script_15s (hook/body/cta), script_30s (hook/body/cta), script_60s (hook/body/cta), cta, platform, objective, why_it_works.

Rules:
- Every hook must include the product name "${productName}"
- Every script section (hook/body/cta) must be a complete, speakable sentence
- 15s script total = under 40 words. 30s = under 85 words. 60s = under 175 words.
- The CTA must be specific — include a URL or action verb + product name
- why_it_works must explain the specific psychological mechanism, not generic benefits

Return JSON: { "concepts": [ ...5 concept objects... ] }`
}
