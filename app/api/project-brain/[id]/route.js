...
export async function GET(req, { params }) {
  try {
    ...
    if (selectError && selectError.code !== 'PGRST116') {
      return NextResponse.json({ error: 'Failed to fetch brain record' }, { status: 500 })
    }
    ...
    if (insertError) {
      return NextResponse.json({ error: 'Failed to create brain record' }, { status: 500 })
    }
...
export async function PATCH(req, { params }) {
  try {
    ...
    // Before upsert, verify ownership of existing record
    ...
    if (error) {
      return NextResponse.json({ error: 'Failed to update brain record' }, { status: 500 })
    }
