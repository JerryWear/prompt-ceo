...
import { buildConsistencyPrompt } from '../../prompt-engine-v3/ad-system/creativeDirector.js'

// POST /api/campaign-consistency
// Checks alignment across the full campaign — angle, hook, voice, music, goal.
// Cost: 1 credit
...
export async function POST(req) {
  try {
    ...
    const campaignState = await req.json()
    if (!campaignState?.productName) {
    ...
    const userPrompt = buildConsistencyPrompt(campaignState)
    ...
    if (!parsed) {
      return NextResponse.json({ status: 'error', message: 'Could not parse consistency report' }, { status: 500 })
    }
