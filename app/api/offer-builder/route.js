...
import { buildOfferBuilderPrompt } from '../../prompt-engine-v3/ad-system/offerBuilder.js'
...
export async function POST(req) {
  try {
    ...
    const prompt = buildOfferBuilderPrompt(adConfig)
    ...
    const grokRes = await fetch('https://api.x.ai/v1/chat/completions', {
      ...
      body: JSON.stringify({
        model: 'grok-3-fast',
        messages: [
          { role: 'system', content: 'You are a world-class offer strategist and direct response marketer. Output ONLY valid JSON. No markdown. Start with {.' },
    ...
    try {
      ...
      offer = JSON.parse(m ? m[0] : raw)
    } catch {
      return NextResponse.json({ status: 'error', message: 'Failed to parse offer' }, { status: 500 })
    }

    await db.from('app_users').update({ credits: userRow.credits - CREDIT_COST }).eq('id', user.id)
    return NextResponse.json({ status: 'success', offer, creditsUsed: CREDIT_COST, creditsRemaining: userRow.credits - CREDIT_COST })
  } catch (err) {
