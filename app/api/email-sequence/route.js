...
import { buildEmailSequencePrompt } from '../../prompt-engine-v3/ad-system/emailSequence.js'
...
export async function POST(req) {
  try {
    ...
    const prompt = buildEmailSequencePrompt(adConfig)
    ...
    const grokRes = await fetch('https://api.x.ai/v1/chat/completions', {
      ...
      body: JSON.stringify({
        model: 'grok-3-fast',
        messages: [
          { role: 'system', content: 'You are a world-class email copywriter. Output ONLY valid JSON. No markdown. Start with {.' },
    ...
    try {
      ...
      sequence = JSON.parse(m ? m[0] : raw)
    } catch {
      return NextResponse.json({ status: 'error', message: 'Failed to parse sequence' }, { status: 500 })
    }
