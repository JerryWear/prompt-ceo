...
import { buildProductDescriptionPrompt } from '../../prompt-engine-v3/ad-system/productDescription.js'
...
export async function POST(req) {
  try {
    ...
    const prompt = buildProductDescriptionPrompt(adConfig)

    const grokRes = await fetch('https://api.x.ai/v1/chat/completions', {
      method: 'POST', headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${apiKey}` },
      body: JSON.stringify({ model: 'grok-3-fast', messages: [
        { role: 'system', content: 'You are a world-class product copywriter. Output ONLY valid JSON. No markdown. Start with {.' },
    ...
    let descriptions
    try { const m = raw.match(/\{[\s\S]*\}/); descriptions = JSON.parse(m ? m[0] : raw) }
    catch { return NextResponse.json({ status: 'error', message: 'Failed to parse descriptions' }, { status: 500 }) }

    await db.from('app_users').update({ credits: userRow.credits - CREDIT_COST }).eq('id', user.id)
    return NextResponse.json({ status: 'success', descriptions, creditsUsed: CREDIT_COST, creditsRemaining: userRow.credits - CREDIT_COST })
  } catch (err) {
