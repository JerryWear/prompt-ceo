...
import { buildVideoAdStoryboardPrompt } from '../../prompt-engine-v3/ad-system/videoStoryboard.js'
...
export async function POST(req) {
  try {
    ...
    const prompt = buildVideoAdStoryboardPrompt(adConfig)
    ...
    const grokRes = await fetch('https://api.x.ai/v1/chat/completions', {
      ...
      body: JSON.stringify({
        model: 'grok-3-fast',
        messages: [
          { role: 'system', content: 'You are a senior video ad director. Output ONLY valid JSON. No markdown. Start with {.' },
    ...
    try {
      ...
      storyboard = JSON.parse(m ? m[0] : raw)
    } catch {
      return NextResponse.json({ status: 'error', message: 'Failed to parse storyboard' }, { status: 500 })
    }

    await db.from('app_users').update({ credits: userRow.credits - CREDIT_COST }).eq('id', user.id)

    return NextResponse.json({ status: 'success', storyboard, creditsUsed: CREDIT_COST, creditsRemaining: userRow.credits - CREDIT_COST })
  } catch (err) {
    console.error('Video Ad Storyboard error:', err)
