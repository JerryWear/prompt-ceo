            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              <div style={{ padding: '16px 20px', borderRadius: 8, background: C.surface, border: `1px solid ${C.hairline}` }}>
                ...
                <div style={{ fontSize: 13, color: C.secondary, lineHeight: 1.7 }}>
                  {currentGroup.id === 'orchestration' && 'Type what you want to build. AI Director understands natural language creative intent — "Create a 30-day luxury hoodie campaign for TikTok." It collects what it needs, then routes to Perfect Day, Full Campaign, Ad Studio, or Life Engine automatically.'}
                  {currentGroup.id === 'life' && 'Choose a day type and world. Life Engine builds 14 complete scene packages — morning to midnight — each with a hook, caption, image prompt, video prompt, wardrobe, lighting, and camera direction. 5 day types: Perfect Day, Creator Day, Travel World, Fitness Day, Ad Campaign Day.'}
                  {currentGroup.id === 'campaign' && 'Same world, same character, same 14-block daily timeline as your Perfect Day — re-engineered to sell. 5 phases: Awareness → Connection → Desire → Conversion → Retargeting. 30-day schedule, 5 retargeting angles, and platform versions all auto-built.'}
                  {currentGroup.id === 'creative' && 'Generate hooks across 5 emotional types — Pain, Desire, Curiosity, Luxury, Direct Offer — CTR-scored before you spend. Angles, captions, video scripts, and full funnel copy all connected to one brief. Image generation with your identity and chosen world.'}
                  {currentGroup.id === 'identity' && 'Upload one photo. Identity System extracts your physical traits — ethnicity, hair, build, age, features. From that point, every image prompt, every video direction, every campaign scene is generated with your consistent character description automatically.'}
                  {currentGroup.id === 'intelligence' && 'Every campaign you run feeds Performance Memory — tracking your best hook types, best worlds, best platforms. Campaign Brain surfaces these patterns as soft biases in future generation. The longer you use it, the smarter it gets about what works for your specific audience.'}
                </div>
              </div>
