...
export function buildSegmentFilters(segments) {
  ...
  // Interleaved pairs required: [v0][a0][v1][a1]...concat=n=N:v=1:a=1
  const interleaved = segments.map((_, i) => `[v${i}][a${i}]`).join('')
  const concatFilter = `${interleaved}concat=n=${segments.length}:v=1:a=1[outv][outa]`
...
export function buildFullFfmpegArgs(plan, inputPath, captionPath, musicPath, outputPath, logoPath = null, introPath = null, outroPath = null, watermarkPath = null) {
  ...
  filters.push(`${interleaved}concat=n=${segs.length}:v=1:a=1[catv][cata]`)
  ...
  // Watermark overlay (top-left, 60% opacity)
  if (hasWatermark) {
  ...
  // Must be declared before the intro/outro block which may reference it
  let finalAudio = '[cata]'

  // Intro/outro video concatenation
