...
// Returns true if user can generate text
// Active subscribers always can. Free users are capped at FREE_PROMPT_LIMIT.
export function canGenerateText(user) {
  if (user?.is_admin) return true
  if (isActive(user)) return true
  return (user?.free_prompts_used || 0) < FREE_PROMPT_LIMIT
}
